...
def get_static_feature_names(data: pd.DataFrame) -> list:
  ...
  # static_feature_names.remove('ClientHash')
  # static_feature_names.remove('ReasonForDischarge_Ongoing')
  # static_feature_names.remove('Date')
...
def main() -> None:
  ...
  client_hash_list = load_client_list(cfg['PATHS']['INFERENCE']['CLIENT_LIST'])
  client_hash_list = client_hash_list['client_hash']
  ...
  def pred_fn(data: np.ndarray):
    static_feat = data[:, static_feature_indices].astype(np.float32)
    ts_feat = data[:, time_series_feature_indices].astype(np.float32)
  ...
  for client_hash in tqdm(client_hash_list):
    client_row = data[data['ClientHash'] == client_hash]
    # client_row = client_row.drop(columns=['ClientHash'])
    
    result = pred_fn(client_row.values)
    predictions.append({
      'ClientHash': client_hash,
      'Prediction': f"{np.round(result[0])}"
    })

  predictions_df = pd.DataFrame(predictions)
  predictions_df.to_csv(cfg['PATHS']['INFERENCE']['RESULTS'], index=False)
